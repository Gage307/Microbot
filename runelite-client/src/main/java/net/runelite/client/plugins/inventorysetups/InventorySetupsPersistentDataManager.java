...
public class InventorySetupsPersistentDataManager
{
	...
	private final net.runelite.client.plugins.microbot.inventorysetups.InventorySetupsCache cache;

	private Gson gson;

	private final List<net.runelite.client.plugins.microbot.inventorysetups.InventorySetup> inventorySetups;
	private final List<net.runelite.client.plugins.microbot.inventorysetups.InventorySetupsSection> sections;
	...
	public InventorySetupsPersistentDataManager(final MInventorySetupsPlugin plugin,
												...
												final Gson gson,
												final List<net.runelite.client.plugins.microbot.inventorysetups.InventorySetup> inventorySetups,
												final List<net.runelite.client.plugins.microbot.inventorysetups.InventorySetupsSection> sections)
	{
		this.plugin = plugin;
	...
	public void loadConfig()
	{
		...
		final List<net.runelite.client.plugins.microbot.inventorysetups.InventorySetup> setupsFromConfig = loadV3Setups();
		inventorySetups.addAll(setupsFromConfig);
		processSetupsFromConfig();

		cleanSetupLayouts();
		Type sectionType = new TypeToken<ArrayList<net.runelite.client.plugins.microbot.inventorysetups.InventorySetupsSection>>()
		{
	...
	public void updateConfig(boolean updateSetups, boolean updateSections)
	{
		if (updateSetups)
		{
			...
			for (final net.runelite.client.plugins.microbot.inventorysetups.InventorySetup setup : inventorySetups)
			{
	...
	private List<net.runelite.client.plugins.microbot.inventorysetups.InventorySetup> loadV1Setups()
	{
		Type setupTypeV1 = new TypeToken<ArrayList<net.runelite.client.plugins.microbot.inventorysetups.InventorySetup>>()
		{
	...
	private List<net.runelite.client.plugins.microbot.inventorysetups.InventorySetup> loadV2Setups()
	{
		...
		List<net.runelite.client.plugins.microbot.inventorysetups.InventorySetup> isList = new ArrayList<>();
	...
	private net.runelite.client.plugins.microbot.inventorysetups.InventorySetup loadV3Setup(String configKey)
	{
		final String storedData = configManager.getConfiguration(CONFIG_GROUP, configKey);
		try
		{
	...
	private List<net.runelite.client.plugins.microbot.inventorysetups.InventorySetup> loadV3Setups()
	{
		final String wholePrefix = ConfigManager.getWholeKey(CONFIG_GROUP, null, CONFIG_KEY_SETUPS_V3_PREFIX);
		...
		List<net.runelite.client.plugins.microbot.inventorysetups.InventorySetup> loadedSetups = new ArrayList<>();
		for (final String configHash : setupsOrder)
		{
			...
			{ // Handles if hash is present only in configOrder.
				final net.runelite.client.plugins.microbot.inventorysetups.InventorySetup setup = loadV3Setup(configKey);
	...
	private void processSetupsFromConfig()
	{
		...
		for (final net.runelite.client.plugins.microbot.inventorysetups.InventorySetup setup : inventorySetups)
		{
			final List<net.runelite.client.plugins.microbot.inventorysetups.InventorySetupsItem> potentialRunePouch = plugin.getAmmoHandler().getRunePouchDataIfInContainer(setup.getInventory());
			if (setup.getRune_pouch() == null && potentialRunePouch != null)
			{
				setup.updateRunePouch(potentialRunePouch);
			}
			final List<net.runelite.client.plugins.microbot.inventorysetups.InventorySetupsItem> potentialBoltPouch = plugin.getAmmoHandler().getBoltPouchDataIfInContainer(setup.getInventory());
			if (setup.getBoltPouch() == null && potentialBoltPouch != null)
			{
				setup.updateBoltPouch(potentialBoltPouch);
			}
			final List<net.runelite.client.plugins.microbot.inventorysetups.InventorySetupsItem> potentialQuiver = plugin.getAmmoHandler().getQuiverDataIfInSetup(setup.getInventory(), setup.getEquipment());
			...
			final String tag = net.runelite.client.plugins.microbot.inventorysetups.InventorySetupLayoutUtilities.getTagNameForLayout(setup.getName());
			plugin.getTagManager().setHidden(tag, true);

			cache.addSetup(setup);

			// add Item names to all the items in the setup.
			net.runelite.client.plugins.microbot.inventorysetups.InventorySetup.getSetupItems(setup).forEach(this::addItemName);
		}
	...
	private void processSetupLayout(final net.runelite.client.plugins.microbot.inventorysetups.InventorySetup setup, final boolean hasMigratedToCoreBTL)
	{
	...
	private void addItemName(final net.runelite.client.plugins.microbot.inventorysetups.InventorySetupsItem item)
	{
...
//			}
	...
	private void migrateSetupItemSlots(final InventorySetup setup)
	{
		...
		if (setup.getInventory() != null)
		{
			...
			{
				net.runelite.client.plugins.microbot.inventorysetups.InventorySetupsItem item = setup.getInventory().get(i);
				if (item != null && !net.runelite.client.plugins.microbot.inventorysetups.InventorySetupsItem.itemIsDummy(item))
				{
					item.setSlot(i);
		...
		if (setup.getEquipment() != null)
		{
			...
			{
				net.runelite.client.plugins.microbot.inventorysetups.InventorySetupsItem item = setup.getEquipment().get(i);
				if (item != null && !net.runelite.client.plugins.microbot.inventorysetups.InventorySetupsItem.itemIsDummy(item))
				{
					item.setSlot(i);
					item.setLocked(false);
		...
		if (setup.getRune_pouch() != null)
		{
			...
			{
				net.runelite.client.plugins.microbot.inventorysetups.InventorySetupsItem item = setup.getRune_pouch().get(i);
				if (item != null && !net.runelite.client.plugins.microbot.inventorysetups.InventorySetupsItem.itemIsDummy(item))
				{
					item.setSlot(i);
					item.setLocked(false);
		...
		if (setup.getBoltPouch() != null)
		{
			...
			{
				net.runelite.client.plugins.microbot.inventorysetups.InventorySetupsItem item = setup.getBoltPouch().get(i);
				if (item != null && !net.runelite.client.plugins.microbot.inventorysetups.InventorySetupsItem.itemIsDummy(item))
				{
					item.setSlot(i);
					item.setLocked(false);
		...
		if (setup.getQuiver() != null)
		{
			...
			{
				net.runelite.client.plugins.microbot.inventorysetups.InventorySetupsItem item = setup.getQuiver().get(i);
